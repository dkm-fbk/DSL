...
from utils import *
import madgrad
from examples.mnistMinusNeg.dataset_generation import *
from examples.mnistMinusNeg.models import *
from examples.mnistMinusNeg.trainer import *
import torch
import random
import numpy as np
import optuna
...
def eval(LOAD_CKPT):
    ...
    for e in range(NUM_VAL_ITERS):
        train_loader, test_loader, mnist_test_data_tsne, mnist_test_data = get_dataset(BATCH_SIZE, BATCH_SIZE_VAL, eval=True)
...
def experiment(optimize_hp, trial=None, args=None):
    ...
    for num_exp in range(NUM_EXPERIMENT):
        ...
        print('Accuracy in minus neg task (before training):' + str(test_sum(model, train_loader, DEVICE)))
        # Learning
